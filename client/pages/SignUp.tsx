...
const useFormValidation = () => {
    ...
    switch (name) {
      ...
      case 'password':
        if (!value) error = 'Password is required';
          ...
          else if (!criteria.uppercase) error = 'Password must contain uppercase letter';
          else if (!criteria.lowercase) error = 'Password must contain lowercase letter';
  ...
  const isFormValid = (formData: Record<string, string>, agreeToTerms: boolean) => {
    const fieldsValid = Object.keys(formData).every(key => formData[key].trim() !== '') &&
...
const AnimatedInput: React.FC<{
  ...
  type, name, placeholder, value, onChange, onBlur, error, touched, icon,
  showPasswordToggle, showPasswordStrength
}) => {
    const [showPassword, setShowPassword] = useState(false);
    const [isFocused, setIsFocused] = useState(false);

    const inputType = showPasswordToggle ? (showPassword ? 'text' : 'password') : type;
    const passwordStrength = showPasswordStrength ? calculatePasswordStrength(value) : null;
        ...
        {showPasswordStrength && value && (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className={`text-sm font-medium ${passwordStrength?.level === 'weak' ? 'text-destructive' :
...
const SignUp: React.FC = () => {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: 'url(https://images.pexels.com/photos/6169641/pexels-photo-6169641.jpeg?auto=compress&cs=tinysrgb&w=1920&h=1280)',
    ...
    >
      ...
      {/* Sign up card */}
      <div className={`w-full max-w-md transition-all duration-500 relative z-10 ${isVisible ? 'animate-scale-in opacity-100' : 'opacity-0 scale-95'
        }`}>
        <div className="bg-card rounded-2xl shadow-xl border border-border px-8 py-6 space-y-4">
          {/* Header */}
          <div className="text-center space-y-2">
            <Link to="/" className="inline-flex items-center space-x-2 text-accent hover:text-accent/80 transition-colors mb-2">
              <Package size={32} />
          ...
          <form onSubmit={handleSubmit} className="space-y-3" noValidate>
            ...
            <button
              ...
              className={`w-full py-3 px-4 rounded-lg font-medium transition-all duration-200 flex items-center justify-center space-x-2 ${isFormValid(formData, agreeToTerms) && !isLoading
