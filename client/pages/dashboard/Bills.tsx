...
// Extended Order type that includes joined customer data
...
export default function Bills() {
  ...
  // Convert orders from database to bills format
  const bills: Bill[] = orders.map((order: any) => {
    // Handle different possible customer data structures
    let customerData = { name: "Guest Customer", phone: "-", email: undefined };

    // Try multiple possible customer data locations
    const customerInfo = order.customers || order.customer || null;

    if (customerInfo) {
      // If customer relationship data is available
      customerData = {
      ...
      // If there's a customer_id but no joined data
      customerData = {
    ...
    console.log("Processing order:", order.id, "Customer data:", customerInfo);
  ...
  console.log("Converted bills:", bills);
  console.log("Sample order structure:", orders[0]); // Log first order to see structure
  ...
  const reprintBill = (bill: Bill) => {
    console.log("reprintBill function called with:", bill);
    try {
      // Format bill data to match BillGenerator interface
      const billData = {
        billNumber: bill.billNumber,
        date: bill.date,
        customer: {
          name: bill.customer.name || "Guest Customer",
        ...
        items: bill.items.map((item) => ({
          ...
          hsnCode: "8517", // Default HSN code for electronics/software
        })),
        subtotal: Number(bill.subtotal) || 0,
        discount: Number(bill.discount) || 0,
        taxRate: Number(bill.taxRate) || 18,
  ...
  const downloadBill = (bill: Bill) => {
    console.log("downloadBill function called with:", bill);
    try {
      // Format bill data to match BillGenerator interface
      const billData = {
        billNumber: bill.billNumber,
      ...
      // Add a small delay to ensure the generator is ready (same as POS)
      setTimeout(() => {
        try {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded p-3 mb-4">
        <p className="text-sm text-yellow-800">
          ...
          {error ? ` Error: ${error}` : ""}
          {!loading && !error
            ? `${bills.length} bills loaded from database, ${filteredBills.length} filtered bills shown`
            : ""}
        </p>
      </div>
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-red-600">Error loading bills</p>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {bills.length === 0
                      ? "No bills have been generated yet. Create some orders in POS to see bills here."
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-between mt-4">
                      <div className="text-sm text-muted-foreground">
                        ...
                        {Math.min(endIndex, filteredBills.length)} of{" "}
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-1 text-sm">
                        <div>
                          <strong>Name:</strong> {selectedBill.customer.name}
                        </div>
                        <div>
                          <strong>Phone:</strong> {selectedBill.customer.phone}
                        </div>
                  ...
                  <div className="flex justify-end space-x-3 pt-4 border-t">
                    ...
                    <Button onClick={() => downloadBill(selectedBill)}>
                      <Download className="w-4 h-4 mr-2" />
                      Download PDF
                    </Button>
                  </div>
                          ...
                          <div className="flex items-center space-x-2">
                            <div className="w-20 bg-muted rounded-full h-2">
                              <div
                                ...
                                style={{
                                  width: `${(data.count / filteredBills.length) * 100}%`,
                                }}
                              ></div>
                            </div>
              ...
              <div className="space-y-3">
                {(() => {
                  const customerRevenue: Record<
                    string,
                    { name: string; total: number; bills: number }
                  > = {};
                  filteredBills.forEach((bill) => {
                    const customerName =
                      ...
                      <div className="flex items-center space-x-2">
                        <div className="w-32 bg-muted rounded-full h-2">
                          <div
                            ...
                            style={{
                              width: `${Math.min((total / Math.max(...Object.values(dailySales))) * 100, 100)}%`,
                            }}
                          ></div>
                        </div>
                        <span className="text-sm font-medium">
